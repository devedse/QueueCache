...
{
    if (Test-Path -LiteralPath (Join-Path $package $folder))
    {
        throw "Obsolete installed folder: $folder"
    }
}
foreach ($file in @('controller/qcache.exe', 'controller/QueueCache.Developer.dll', 'desktop/QueueCache.Desktop.exe'))
{
    ...
    {
        throw "Missing packaged application component: $file"
    }
}
if (@(Get-ChildItem -LiteralPath $package -Filter 'qcache-*-tests.exe' -Recurse).Count)
{
    throw 'Standalone test executable leaked into package.'
}
