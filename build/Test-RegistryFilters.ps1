...
{
    throw 'Expected installer verifier and filter-order function.'
}
...
function Expect-Rejection([scriptblock]$Check)
{
    ...
    if (-not $rejected)
    {
        throw 'Verifier accepted an invalid registry value.'
    }
}
try
{
        ...
        ))
    {
        ...
        if (($ordered -join '|') -cne ($case.After -join '|'))
        {
            throw 'Incorrect class-filter ordering.'
        }
