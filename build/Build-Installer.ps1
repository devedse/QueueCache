...
{
    throw 'Installer requires an explicitly test-signed write-cache package.'
}
if (-not $Compiler)
{
...
{
    throw 'Install Inno Setup 6 or specify -Compiler.'
}
& $Compiler "/DPackageDir=$package" "/DBuildVersion=$($metadata.version)" "$root/packaging/QueueCache.iss"
if ($LASTEXITCODE)
{
    throw "Installer compiler failed: $LASTEXITCODE"
}
