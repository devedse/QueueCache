...
struct Node { Node* Next; unsigned Kind; unsigned Range; }; // 1 read, 2 write, 3 fence
struct Queue {
    Node Nodes[145]{};
    ...
    constexpr Node* Step(bool& more) { Visits = 0; return Scan.Step(&Nodes[0], *this, 64, more); }
};
constexpr bool DeepQueue() {
    Queue q; q.Nodes[140].Kind = 1; q.Nodes[140].Range = 2;
    bool more = false;
    for (unsigned i = 0; i < 8; ++i) {
        auto result = q.Step(more);
        if (q.Visits > 64) return false;
        if (result) return result == &q.Nodes[140] && i >= 4;
        if (!more) return false;
    }
    return false;
}
constexpr bool OlderWriteAndFence() {
    Queue q; q.Nodes[140].Kind = 1; // Conflicts with older writes.
    bool more = false;
    for (unsigned i = 0; i < 8; ++i) { if (q.Step(more)) return false; if (!more) break; }
    if (more) return false;
    q.Scan.Reset(q.Nodes[0].Next);
    q.Nodes[140].Range = 2; q.Nodes[70].Kind = 3;
    if (q.Step(more) || !more) return false;
    if (q.Step(more) || more) return false;
    if (q.Step(more) || more) return false; // Stable fence doesn't spin.
    q.Remove(70); // Cancellation opens the cursor, without a new insertion.
    for (unsigned i = 0; i < 8; ++i) { auto result = q.Step(more); if (result) return result == &q.Nodes[140]; if (!more) return false; }
    return false;
}
constexpr bool CancelAndReinsert() {
    Queue q; q.Nodes[140].Kind = 1; q.Nodes[140].Range = 2;
    bool more = false;
    q.Step(more); q.Step(more); q.Step(more); // Candidate is being validated.
    if (q.Scan.Candidate != &q.Nodes[140]) return false;
    q.Remove(140);
    if (q.Scan.Candidate) return false;
    if (q.Step(more) || more) return false;
    ...
    q.Nodes[140].Next = q.Nodes[0].Next; q.Nodes[0].Next = &q.Nodes[140];
    q.Nodes[140].Kind = 2;
    q.Scan.Reset(q.Nodes[0].Next);
    q.Nodes[144].Kind = 1; q.Nodes[144].Range = 2;
    for (unsigned i = 0; i < 10; ++i) { if (q.Step(more)) return false; if (!more) return true; }
    return false;
}
constexpr bool CursorRemovalAndAppend() {
    Queue q; bool more = false;
    q.Step(more); // Find points at 65.
    q.Remove(65);
    if (q.Scan.Find != &q.Nodes[66]) return false;
    while (more) if (q.Step(more)) return false;
    // Reuse removed node as a new tail read.
    q.Nodes[144].Next = &q.Nodes[65]; q.Nodes[65].Next = &q.Nodes[0];
    q.Nodes[65].Kind = 1; q.Nodes[65].Range = 2;
    q.Scan.Appended(&q.Nodes[0], &q.Nodes[65]);
    for (unsigned i = 0; i < 8; ++i) { auto result = q.Step(more); if (result) return result == &q.Nodes[65]; if (!more) return false; }
    return false;
}
constexpr bool LateDependencyAndValidationCancellation() {
    ...
    for (unsigned i = 0; i < 10; ++i) { if (q.Step(more)) return false; if (!more) break; }
    if (more) return false;
    q.Remove(120); q.Scan.Reset(q.Nodes[0].Next);
    q.Step(more); q.Step(more); q.Step(more);
    if (!q.Scan.Candidate || !q.Scan.Check) return false;
    ...
    unsigned index = 1;
    while (&q.Nodes[index] != check) ++index;
    auto successor = check->Next;
    q.Remove(index);
    if (q.Scan.Check != successor) return false;
    for (unsigned i = 0; i < 8; ++i) { auto result = q.Step(more); if (result) return result == &q.Nodes[140]; if (!more) return false; }
    return false;
}
constexpr bool NewPrefixInvalidatesValidation() {
    ...
    q.Remove(144);
    q.Nodes[144].Kind = 3;
    q.Nodes[144].Next = q.Nodes[0].Next; q.Nodes[0].Next = &q.Nodes[144];
    q.Scan.Reset(q.Nodes[0].Next);
