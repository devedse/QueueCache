...
template<class Node> struct QcReadSelection {
    ...
    template<class Queue> constexpr Node* Step(Node* head, Queue& queue,
        unsigned budget, bool& more) {
        more = false;
        while (budget) {
            if (Candidate) {
                if (Check == Candidate) {
