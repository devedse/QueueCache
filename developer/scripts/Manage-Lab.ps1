...
function InspectFilter([string]$instanceId)
{
    ...
    if ($LASTEXITCODE)
    {
        throw 'Unable to inspect target devnode filters.'
    }
...
try
{
    ...
    if ($Action -eq 'EnableTestSigning')
    {
        ...
        {
            throw 'Confirm a current VM snapshot with -SnapshotConfirmed.'
        }
        if (Confirm-SecureBootUEFI)
        {
            throw 'Disable Secure Boot in the VM firmware first. Do not delete EFI disk/keys.'
        }
        $protected = @(Get-BitLockerVolume | Where-Object { $_.ProtectionStatus -ne 'Off' })
        if ($protected.Count)
        {
            throw 'BitLocker protection is active. Secure recovery keys and review protection before proceeding.'
        }
    ...
    if ($Action -eq 'Status')
    {
        ...
        if ($DiskNumber -ge 0)
        {
            Native "$package/controller/qcache.exe" @('status', "PhysicalDrive$DiskNumber", '--json')
        }
        return
    }
    if ($Action -eq 'Uninstall')
    {
        ...
        {
            throw 'No recorded installation. Do not guess an instance ID.'
        }
    ...
    if (-not $SnapshotConfirmed)
    {
        throw 'Confirm a current whole-VM snapshot with -SnapshotConfirmed.'
    }
    if ($DiskNumber -le 0 -or $ExpectedBytes -le 0)
    {
        throw 'Specify a nonzero secondary disk number and exact expected size.'
    }
    $upgrade = $Action -eq 'Upgrade'
    $reattach = $Action -in @('Reattach', 'Upgrade')
    if ($AllowFormattedDisk -and -not $reattach -and -not $ExpectedInstanceId)
    {
        throw 'Initial formatted-disk attachment requires an explicit expected PnP identity.'
    }
    if ((Test-Path $stateFile) -and -not $reattach)
    {
        throw 'Installation state already exists. Inspect it; do not overwrite recovery information.'
    }
    if ($reattach -and -not (Test-Path $stateFile))
    {
        throw 'Reattachment requires the recorded original installation.'
    }
    if (Confirm-SecureBootUEFI)
    {
        throw 'Secure Boot remains enabled. Complete the firmware/test-signing prerequisites first.'
    }
...
'@
    if (-not [LabCodeIntegrity]::TestSigningActive())
    {
        throw 'Test-signing is not active in this boot. Enable it and reboot first.'
    }
    ...
    {
        throw 'Cannot uniquely resolve disk number to PnP identity.'
    }
    $instanceId = $identities[0].PNPDeviceID
    if ($ExpectedInstanceId -and $ExpectedInstanceId -ne $instanceId)
    {
        throw 'Disk PnP identity differs from the explicitly selected target.'
    }
    $letters = @(Get-Partition -DiskNumber $DiskNumber | Where-Object DriveLetter | ForEach-Object { "$($_.DriveLetter):" })
    if (@(Get-CimInstance Win32_PageFileUsage | Where-Object { $_.Name.Substring(0, 2) -in $letters }).Count)
    {
        throw 'Paging disk attachment is unsupported.'
    }
    $before = InspectFilter $instanceId
    if (@($before.UpperFilters).Count)
    {
        throw 'Initial lab install requires no existing per-device upper filters; review manually instead.'
    }
    ...
    if ($reattach)
    {
        ...
        if ($saved.InstanceId -ne $instanceId -or $saved.DriverKey -ne $before.DriverKey)
        {
            throw 'Reattachment identity differs from the recorded disk.'
        }
        if ((Get-Service qcachelab).Status -ne 'Stopped')
        {
            throw 'Reboot after removal before reattaching.'
        }
        $service = Get-ItemProperty $servicePath
        if ($service.LabAllowedDriverKey -ne $before.DriverKey -or $service.Start -ne 3 -or
            $service.ImagePath -ne 'System32\drivers\qcachelab.sys')
        {
            throw 'Unexpected retained service configuration.'
        }
        if (-not $upgrade -and (Get-FileHash $binary).Hash -ne (Get-FileHash "$package/driver/qcachelab.sys").Hash)
        {
            throw 'Retained driver differs from the signed package. Use explicit Upgrade only after removal/reboot.'
        }
    }
    elseif ((Test-Path $servicePath) -or (Test-Path $binary))
    {
        throw 'Existing qcachelab service/binary: review before installation.'
    }
    $metadata = Get-Content "$package/build-info.json" -Raw | ConvertFrom-Json
    if ((-not $metadata.labPassThrough -and -not ($metadata.labWriteCache -and $AllowWriteCache)) -or -not $metadata.driverSigned)
    {
        throw 'Expected signed lab package; write-cache builds require explicit -AllowWriteCache.'
    }
    foreach ($line in (Get-Content "$package/SHA256SUMS.txt"))
    {
        $parts = $line -split '  ', 2
        $path = [IO.Path]::GetFullPath((Join-Path $package $parts[1]))
        if (-not $path.StartsWith([IO.Path]::GetFullPath($package).TrimEnd('\') + '\', [StringComparison]::OrdinalIgnoreCase))
        {
    ...
    {
        throw "Invalid signature: $($signature.Status)"
    }
    ...
    if ($disk.IsBoot -or $disk.IsSystem -or $disk.Size -ne $ExpectedBytes -or $disk.PartitionStyle -ne $originalStyle -or
        ...
        $again.Count -ne 1 -or $again[0].PNPDeviceID -ne $instanceId)
    {
        throw 'Disk identity changed during preflight.'
    }
    if ($upgrade)
    {
        ...
        if ((Get-Service qcachelab).Status -ne 'Stopped')
        {
            throw 'Service changed state before upgrade.'
        }
        ...
        {
            Copy-Item -LiteralPath $binary -Destination $backup
        }
        if ((Get-FileHash -LiteralPath $backup).Hash -ne $oldHash)
        {
            throw 'Previous binary backup verification failed.'
        }
        ...
        if ((Get-FileHash $binary).Hash -ne (Get-FileHash "$package/driver/qcachelab.sys").Hash)
        {
            throw 'Installed binary hash verification failed.'
        }
    }
    if (-not $reattach)
    {
    ...
    {
        Log 'Reusing verified stopped service and original recovery state.'
    }
...
{
    Stop-Transcript | Out-Null; Write-Host "Log: $log"
}
