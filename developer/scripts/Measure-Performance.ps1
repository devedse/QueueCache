...
function Qc
{
    ...
    try
    {
        ...
        {
            throw "qcache failed ($LASTEXITCODE): $result"
        }
        $result
    }
    finally
...
function Get-State
{
    Qc 'policy' 'status' $Volume '--json' | ConvertFrom-Json
}
...
function Assert-Target
{
    ...
    if (-not $target.DriveLetter)
    {
        throw "$Volume is not mounted."
    }
    $system = (Get-Item $env:SystemRoot).PSDrive.Name + ':'
    if ($Volume -ieq $system)
    {
        throw "Refusing to benchmark the Windows volume ($system)."
    }
    $partition = Get-Partition -DriveLetter $letter
    $disk = Get-Disk -Number $partition.DiskNumber
    if ($disk.IsBoot -or $disk.IsSystem -or $disk.Number -eq 0)
    {
        throw "Refusing to benchmark boot/system disk $($disk.Number)."
    }
    if (-not (Test-Path -LiteralPath $DiskSpd))
    {
        throw "DiskSpd not found: $DiskSpd"
    }
    $state = Get-State
    if (-not $state)
    {
        throw "No QueueCache state for $Volume. Install the driver and restart Windows first."
    }
...
function Apply-Config([string]$config, [int]$parallelism = 1)
{
    if ($config -eq 'off')
    {
        Qc 'policy' 'pause' $Volume '--runtime-only' | Out-Null
    }
    else
    {
    ...
    if ($state.RuntimeStatus -ne $expected)
    {
        throw "policy $config did not reach $expected (got $($state.RuntimeStatus))"
    }
    $state
}
...
function New-TestFile([string]$name, [int]$sizeMiB)
{
    ...
    if (-not (Test-Path $path) -or (Get-Item $path).Length -ne ($sizeMiB * 1MB))
    {
        ...
        if ($LASTEXITCODE -ne 0)
        {
            throw "DiskSpd file creation failed ($LASTEXITCODE)."
        }
...
function Read-DiskSpd([string]$text)
{
    ...
    if ($total.Count)
    {
        $fields = @($total[0] -split '\|' | ForEach-Object { $_.Trim() })
    }
    $result = [ordered]@{
        MiBps    = if ($fields.Count -gt 2)
        {
            [double]$fields[2]
        }
        else
    ...
    foreach ($p in '50th', '95th', '99th', '3-nines', 'max')
    {
        ...
        if ($row.Count)
        {
            $cols = @($row[0] -split '\|' | ForEach-Object { $_.Trim() }); $value = $cols[-1]
        }
        $result["Lat_$($p -replace '-', '')"] = $value
    }
    if ($null -eq $result.IOPS)
    {
        throw "DiskSpd report has no parseable total."
    }
    if ($result.IOPS -eq 0)
    {
...
function Invoke-DiskSpd([string]$label, [string[]]$spdArgs, [string]$path)
{
    ...
    if ($exitCode -ne 0)
    {
        throw "DiskSpd $label failed ($exitCode); see raw report."
    }
    Read-DiskSpd $text
}
...
function Finish-Load($process)
{
    ...
    {
        throw "Background DiskSpd timed out (PID $($process.Id))."
    }
    if ($process.ExitCode -ne 0)
    {
        throw "Background DiskSpd failed ($($process.ExitCode))."
    }
}
...
{
    $before = Get-State
    if ($before.SupportsPerformance)
    {
        Qc 'developer' 'performance' $Volume | Set-Content (Join-Path $OutputDirectory "$label-performance-before.json")
    }
    ...
    if ($before.SupportsPerformance)
    {
        Qc 'developer' 'performance' $Volume | Set-Content (Join-Path $OutputDirectory "$label-performance-after.json")
    }
...
function Measure-Flush([hashtable]$fields)
{
    ...
    $all = @{} + $fields
    $all.MiBps = if ($sw.Elapsed.TotalSeconds -gt 0)
    {
        [math]::Round($drained / $sw.Elapsed.TotalSeconds, 2)
    }
    else
    {
        $null
...
function Warm-ReadSet([string]$path)
{
    ...
    if ($LASTEXITCODE -ne 0)
    {
        throw "DiskSpd warmup failed ($LASTEXITCODE)."
    }
    $state = Get-State
...
$capacityFile = if ($Experiments -contains 'capacity')
{
    New-TestFile 'capacity.dat' $CapacitySetMiB
}
else
{
    $null
...
try
{
    foreach ($repeat in 1..$Repeats)
    {
        foreach ($config in $Configs)
        {
            ...
            {
                $delays = @(0)
                if ($Experiments -contains 'slow-storage')
                {
                    $delays += $SlowStorageDelayMs
                }
                if ($Experiments -notcontains 'interference')
                {
                    $delays = @($SlowStorageDelayMs)
                }
                foreach ($delay in $delays)
                {
                    $experiment = if ($delay -eq 0)
                    {
                        'interference'
                    }
                    else
                    ...
                    if ($delay -gt 0)
                    {
                        Qc 'lab-delay' $Volume $delay | Out-Null; Say "lab-delay $delay ms enabled"
                    }
                    ...
                    if ($delay -gt 0)
                    {
                        Qc 'lab-delay' $Volume 0 | Out-Null; Say 'lab-delay cleared'
                    }
            ...
            if ($Experiments -contains 'capacity')
            {
                ...
                if ($row.ThrottleWaits -eq 0)
                {
                    Say "WARNING: capacity experiment did not throttle (accepted $($row.AcceptedMiB) MiB, budget $BudgetMiB MiB). Increase CapacitySetMiB or Duration."
                }
            ...
            if ($Experiments -contains 'flush-under-load')
            {
                ...
                {
                    $writer.WaitForExit(120000) | Out-Null
                }
...
finally
{
    ...
    try
    {
        ...
        if ($original.BudgetBytes -gt 0)
        {
            ...
            if (-not $o)
            {
                throw 'Cannot faithfully restore an older driver without full policy options.'
            }
            $restoreArgs = @('policy', 'apply', $Volume, '--runtime-only', '--budget-mib', $wanted,
                '--preset', $(if ($original.UnsafeDefer)
                    {
                        'Fast'
                    }
                    else
            ...
            if (-not $o.RetainWrites)
            {
                $restoreArgs += '--discard-drained'
            }
            if (-not $o.PromoteOnRead)
            {
                $restoreArgs += '--no-promotion'
            }
        ...
        {
            Qc 'policy' 'remove' $Volume '--runtime-only' | Out-Null
        }
    }
    catch
    {
