...
{
    throw 'Recorded disk identity is not unique/present.'
}
$disk = Get-Disk -Number $matches[0].Index
if ($disk.Number -le 0 -or $disk.IsBoot -or $disk.IsSystem -or $disk.PartitionStyle -ne 'RAW')
{
    throw 'Expected RAW secondary disk.'
}
$cli = Join-Path $PackageDirectory 'controller/qcache.exe'
$json = & $cli lab-filter inspect $state.InstanceId
if ($LASTEXITCODE)
{
    throw 'Device inspection failed.'
}
$properties = $json | ConvertFrom-Json
if ($properties.DriverKey -ne $state.DriverKey)
{
    throw 'Driver key changed.'
}
$registered = @($properties.UpperFilters) -contains 'qcachelab'
if ($registered -ne ($Mode -eq 'attached'))
{
    throw 'Filter registration does not match requested test phase.'
}
...
try
{
    ...
    if ($LASTEXITCODE)
    {
        throw "Read test failed: $LASTEXITCODE"
    }
}
finally
{
