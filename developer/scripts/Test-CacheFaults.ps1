...
if ($devices.Count -ne 1 -or $devices[0].Index -le 0)
{
    throw 'Recorded secondary disk identity is not unique.'
}
$disk = Get-Disk -Number $devices[0].Index
if ($disk.IsBoot -or $disk.IsSystem -or $disk.PartitionStyle -ne 'RAW' -or $disk.NumberOfPartitions)
{
    throw 'Expected empty RAW secondary disk.'
}
...
function InvokeCacheCommand([string[]]$Arguments, [int]$Expected = 0)
{
    ...
    {
        throw "Unexpected qcache exit: $LASTEXITCODE"
    }
}
function State
{
    ...
    if ($LASTEXITCODE)
    {
        throw 'Cannot obtain coherent cache snapshot.'
    }
    $json | Write-Host
...
function Workload([string]$Mode, [int]$Expected, [long]$Prefix = 0)
{
    ...
    if ($Prefix)
    {
        $arguments += @('--prefix-bytes', [string]$Prefix)
    }
    ...
    if ($LASTEXITCODE -ne $Expected)
    {
        throw "Unexpected test exit $LASTEXITCODE (expected $Expected)."
    }
}
...
try
{
    ...
    foreach ($mode in 1, 2, 4, 5)
    {
        ...
        $stillFaulted = State
        if ($stillFaulted.DirtyBytes -ne $fault.DirtyBytes -or -not $stillFaulted.Faulted)
        {
            throw 'Flush silently discarded faulted data.'
        }
    ...
    Workload '--write-disposable-region' 1
    $flushFault = State
    if (-not $flushFault.Faulted -or -not $flushFault.LastError)
    {
        throw 'Flush error was not visible.'
    }
    InvokeCacheCommand @('flush', $target) 1
    InvokeCacheCommand @('retry', $target)
    ...
    $final = State
    if ($final.Enabled -or $final.Faulted -or $final.DirtyBytes -or $final.InFlightBytes)
    {
        throw 'Final disable did not leave a clean cache.'
    }
    foreach ($mode in 6, 7)
    {
