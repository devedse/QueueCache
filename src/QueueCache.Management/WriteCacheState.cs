    ...
    ulong Flushes, ulong PeakDirtyBytes)
{
    ...
    public CachePerformance? Performance
    {
        get; init;
    }
    public bool SupportsPerformance => (Flags & 2048) != 0;
    public ulong CleanReadBytes
    {
        get; init;
    }
    public ulong CleanWriteBytes
    {
        get; init;
    }
    public ulong ReadHitBytes
    {
        get; init;
    }
    public ulong ReadMissBytes
    {
        get; init;
    }
    ...
    public ulong GlobalLimitBytes
    {
        get; init;
    }
    public ulong GlobalReservedBytes
    {
        get; init;
    }
    ...
    public static WriteCacheState DecodeReadWrite(ReadOnlySpan<byte> data)
    {
        if (data.Length != ReadWriteWireSize || BinaryPrimitives.ReadUInt32LittleEndian(data) != 3 ||
            BinaryPrimitives.ReadUInt32LittleEndian(data[4..]) != ReadWriteWireSize)
            throw new InvalidDataException("Unsupported read/write cache state.");
        Span<byte> legacy = stackalloc byte[ExtendedWireSize];
        data[..ExtendedWireSize].CopyTo(legacy);
        BinaryPrimitives.WriteUInt32LittleEndian(legacy, 2);
        BinaryPrimitives.WriteUInt32LittleEndian(legacy[4..], ExtendedWireSize);
        var v = new ulong[10];
        for (int i = 0; i < v.Length; i++)
            v[i] = BinaryPrimitives.ReadUInt64LittleEndian(data[(208 + i * 8)..]);
        var state = DecodeExtended(legacy) with
        {
            Options = CacheOptions.Decode(data.Slice(160, 48)),
            CleanReadBytes = v[0],
            CleanWriteBytes = v[1],
            ReadHitBytes = v[2],
            ReadMissBytes = v[3],
            ...
            Generation = v[6],
            Instance = v[7],
            GlobalLimitBytes = v[8],
            GlobalReservedBytes = v[9]
        };
    ...
    public ulong DiscardedBytes
    {
        get; init;
    }
    public ulong LowerWrites
    {
        get; init;
    }
    public ulong BatchedWrites
    {
        get; init;
    }
    public ulong TrimRequests
    {
        get; init;
    }
    public bool ExtendedCountersAvailable
    {
        get; init;
    }
    ...
    public static WriteCacheState DecodeExtended(ReadOnlySpan<byte> data)
    {
        ...
        Span<byte> legacy = stackalloc byte[WireSize];
        data[..WireSize].CopyTo(legacy);
        BinaryPrimitives.WriteUInt32LittleEndian(legacy, 1);
        BinaryPrimitives.WriteUInt32LittleEndian(legacy[4..], WireSize);
        return Decode(legacy) with
        {
            ...
            TrimRequests = BinaryPrimitives.ReadUInt64LittleEndian(data[152..]),
    ...
    public static WriteCacheState Decode(ReadOnlySpan<byte> data)
    {
        ...
        for (var i = 0; i < values.Length; i++)
            values[i] = BinaryPrimitives.ReadUInt64LittleEndian(data[(16 + i * 8)..]);
