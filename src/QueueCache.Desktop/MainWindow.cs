...
public sealed class MainWindow : Window
{
    ...
    public MainWindow(ICacheTaskService service)
    {
        ...
        controls.Children.Add(Action("Refresh disks", Refresh));
        Grid.SetColumn(controls, 1);
        heading.Children.Add(controls);
        var body = new StackPanel { Margin = new Thickness(36), Spacing = 12 };
        body.Children.Add(heading);
        body.Children.Add(Text("DISKS & CACHES", 12, Muted, FontWeight.SemiBold));
    ...
    private async Task Refresh()
    {
        ...
        try
        {
            var disks = await service.ListAsync();
            if (closed)
                return;
            ...
            if (!views.Any(v => v.Busy) && signature != string.Join("|", views.Select(v => v.Disk.Instance + string.Join(",", v.Disk.Volumes))))
            {
                ...
                {
                    var card = new Card(disk);
                    views.Add(card);
                    cards.Children.Add(BuildCard(card));
                }
            }
            await Sample();
            message.Text = disks.Count == 0 ? "No disks found." : "";
        }
    ...
    private Control BuildCard(Card card)
    {
        ...
        card.Settings = DiskAction("Add cache", () => Edit(card));
        actions.Children.Add(card.Settings);
        card.Settings.Background = Accent;
        card.Settings.Foreground = Brushes.White;
        card.Pause = DiskAction("Pause", () => service.SetEnabledAsync(Volume(card), !(card.State?.Enabled ?? false), Persistent(card)));
        actions.Children.Add(card.Pause);
        card.Flush = DiskAction("Flush now", () => service.FlushAsync(card.Disk));
        ...
        card.Remove = DiskAction("Remove cache", async () => { message.Text = "Draining and removing cache…"; await service.RemoveAsync(Volume(card)); });
        ...
        tools.Children.Add(DiskAction("File tests", () => Test(card, false)));
        tools.Children.Add(DiskAction("Benchmark", () => Test(card, true)));
    ...
    private async Task SampleCard(Card card)
    {
        ...
        try
        {
            var state = await service.ReadAsync(card.Disk);
            ...
            card.Settings.IsEnabled = !card.Busy && state.SupportsReadWrite && card.Disk.Volumes.Length > 0;
            ...
            if (state.Faulted)
                card.Description.Text = $"Disk I/O failed (0x{state.LastError:X8}). Pending writes are retained; check the disk before retrying.";
        }
        catch (Exception ex)
        {
            card.State = null;
            card.Badge.Text = "Not connected";
            card.Description.Text = "Driver unavailable. Finish installation and restart Windows, then refresh.";
            card.Activity.IsVisible = false;
