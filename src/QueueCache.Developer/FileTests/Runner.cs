...
public static class Runner
{
    public static async Task<int> RunAsync(string[] args)
    {
        ...
        try
        {
            ...
            long length = 64L << 20;
            if (args[4] is "--dirty-reboot" or "--dirty-reboot-unsafe" or "--test-flush-policy" or "--test-coalescing" && args.Length != 5)
                throw new ArgumentException("Policy/reboot/coalescing tests use a fixed 64 MiB file size.");
            if (writing && args.Length == 6)
            {
                if (!int.TryParse(args[5], out var mib) || mib < 64 || mib > 8192)
                    throw new ArgumentException("File size must be 64..8192 MiB.");
                length = (long)mib << 20;
            }
            ...
            if (writing)
            {
                ...
                if (new DriveInfo(root).AvailableFreeSpace < length * 2 + (1L << 30))
                    throw new IOException("Need space for both files plus 1 GiB headroom.");
                if (Directory.Exists(directory) || File.Exists(directory))
                    throw new IOException("New test directory already exists.");
            ...
            if (writing)
            {
                ...
                try
                {
                    ...
                    for (long offset = 0; offset < length; offset += blockBytes)
                    {
                    ...
                    {
                        stage = "source-written",
                        runId,
                        length,
                        writeReturnSeconds = writeReturned,
                        includingFileFlushSeconds = watch.Elapsed.TotalSeconds
                    }));
                }
                finally
                {
                    // Never leave a writer running after the parent reports failure.
                    if (concurrentCopy is not null)
                        await concurrentCopy;
                }
                if (concurrentCopy is null)
                    File.Copy(source, temporaryCopy, overwrite: false);
                ...
                using (var stream = new FileStream(source, FileMode.Open, FileAccess.Write, FileShare.None))
                {
                    ...
                    var overwrite = new byte[4096];
                    Pattern(overwrite, overwriteOffset, seed);
                ...
                Console.WriteLine(JsonSerializer.Serialize(new
                {
                    stage = "files-written-and-volume-flushed",
                    runId,
            ...
            else
            {
                ...
                if (new FileInfo(manifestPath).Length > 16384)
                    throw new IOException("Manifest exceeds the lab metadata limit.");
            ...
            foreach (var (file, expected) in new[] { (source, manifest.SourceHash), (copy, manifest.CopyHash) })
            {
                ...
                if (stream.Length != length || Convert.ToHexString(SHA256.HashData(stream)) != expected)
                    throw new IOException($"File integrity mismatch: {file}");
            }
            if (writing)
                volume.Flush();
            ...
            if (args[4] == "--test-coalescing")
            {
                ...
                if (initial.PayloadCapacity < 128UL << 20)
                    throw new IOException("Coalescing test requires at least 128 MiB payload capacity.");
                ...
                if (queued.DirtyBytes == 0)
                    throw new IOException("No pending payload for the completion-error/retry test.");
                ...
                for (var i = (1 << 20) + 4096; i < (1 << 20) + 8192; i++)
                    payload[i] ^= 0xA7;
                UnbufferedFileWrite.WritePrefix(source, payload, flush: true);
                volume.Flush();
                using (var restored = File.OpenRead(source))
                    if (Convert.ToHexString(SHA256.HashData(restored)) != manifest.SourceHash)
                        throw new IOException("Restored oracle mismatch.");
                Console.WriteLine(JsonSerializer.Serialize(new
                {
                    result = "PASS",
                    mode = args[4],
                    ...
                    queued,
                    final,
                    coalescedSourceHash = HashOracle(finalSeed, false, length)
                }));
                return 0;
            }
            if (args[4] is "--dirty-reboot" or "--dirty-reboot-unsafe" or "--test-flush-policy")
            {
                ...
                {
                    Version = 2,
                    SourceHash = HashOracle(manifest.Seed, true, length, true)
                };
                using (var metadata = new FileStream(manifestPath, FileMode.Create, FileAccess.Write, FileShare.None))
                {
                    JsonSerializer.Serialize(metadata, manifest);
                    metadata.Flush(true);
                }
                volume.Flush();
                using var control = new CacheDevice($"PhysicalDrive{number}", writable: true);
                ...
                if (testUnsafe)
                {
                    ...
                    control.Control(WriteCacheAction.FlushPolicy, value: 1);
                    control.Control(WriteCacheAction.Enable);
                    if (!control.GetWriteCacheState().UnsafeDefer)
                        throw new IOException("Unsafe policy was not selected.");
                }
                var diagnosticsBefore = testUnsafe ? control.GetDiagnostics() : null;
                control.Control(WriteCacheAction.LabDelay, value: 2000);
                var prefix = new byte[2 << 20];
                Pattern(prefix, 0, manifest.Seed);
                for (var i = (1 << 20) + 4096; i < (1 << 20) + 8192; i++)
                    prefix[i] ^= 0xA7;
                ...
                if (dirty.DirtyBytes == 0 || dirty.Faulted)
                    throw new IOException("No outstanding dirty payload observed; cannot claim a dirty-reboot test.");
                if (testUnsafe)
                {
                    ...
                    Console.WriteLine(JsonSerializer.Serialize(new
                    {
                        stage = "deferred-flush-and-write-through",
                        dirty,
                        observed
                    }));
                }
                if (args[4] == "--test-flush-policy")
                {
                    ...
                    if (!switchRejected || !control.GetWriteCacheState().UnsafeDefer)
                        throw new IOException("Policy changed while enabled/dirty.");
                    control.Control(WriteCacheAction.LabDelay, value: 0);
                    ...
                    if (!flushFailed || !control.GetWriteCacheState().Faulted)
                        throw new IOException("Manual flush masked lower failure.");
                    control.Control(WriteCacheAction.Retry);
                    control.Control(WriteCacheAction.Disable);
                    control.Control(WriteCacheAction.FlushPolicy, value: 0);
                    control.Control(WriteCacheAction.Enable);
                    ...
                    Console.WriteLine(JsonSerializer.Serialize(new
                    {
                        ...
                        directory,
                        final,
                        diagnostics = control.GetDiagnostics(),
                        manifest
                    }));
                    return 0;
                }
                Console.WriteLine(JsonSerializer.Serialize(new
                {
                    result = "READY_FOR_NORMAL_REBOOT",
                    runId,
                    directory,
                    dirty,
                    manifest,
        ...
        static void RejectReparse(string path)
        {
            if ((File.GetAttributes(path) & FileAttributes.ReparsePoint) != 0)
                throw new IOException("Reparse points are not test targets.");
        }
        static void WriteAndVerifyPatternFile(string path, ulong seed, long length)
        {
            ...
            if (Convert.ToHexString(SHA256.HashData(read)) != HashOracle(seed, false, length))
                throw new IOException("Concurrent reader integrity mismatch.");
        }
        ...
        static string HashOracle(ulong seed, bool overwrite, long length, bool dirtyPrefix = false)
        {
            ...
            {
                Pattern(block, offset, seed);
                if (overwrite && offset == (1 << 20))
                {
                    for (var i = 4096; i < 8192; i++)
                    {
                        block[i] ^= 0xA7;
                    }
                }
                if (dirtyPrefix && offset < (2 << 20))
                {
...
internal sealed class CheckedVolume : IDisposable
{
    ...
    public CheckedVolume(char letter, int disk, long expectedDiskBytes, bool writable)
    {
        ...
        try
        {
            ...
            if (!DeviceIoControl(handle, 0x560000, IntPtr.Zero, 0, extents, (uint)extents.Length, out var returned, IntPtr.Zero))
                throw new Win32Exception();
