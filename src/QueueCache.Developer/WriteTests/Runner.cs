...
public static class Runner
{
    public static async Task<int> RunAsync(string[] args)
    {
        ...
        {
            if (!ulong.TryParse(args[^1], out requestedSeed) || requestedSeed == 0)
                return 2;
            args = args[..^2];
        }
        ...
        if (args[3] is "--write-dirty-prefix" or "--verify-dirty-prefix")
        {
            if (requestedSeed != 0 || args.Length != 6 || !int.TryParse(args[4], out prefixBytes) || prefixBytes <= 0 || prefixBytes > regionLength || prefixBytes % blockSize != 0 ||
                !ulong.TryParse(args[5], out patternSeed) || patternSeed == 0)
                return 2;
        }
        else if (args[3] == "--verify-base-prefix")
        {
            if (args.Length != 5 || !int.TryParse(args[4], out prefixBytes) || prefixBytes <= 0 || prefixBytes > regionLength || prefixBytes % blockSize != 0)
                return 2;
        }
        ...
        try
        {
                        ...
                        foreach (var enabled in round % 2 == 0 ? new[] { false, true } : new[] { true, false })
                        {
                            ...
                            for (var i = 0; i < size; i += blockSize)
                                if (disk.Read(regionStart + i, blockSize).AsSpan().SequenceEqual(oracle.AsSpan(i, blockSize)))
                                    throw new IOException("Performance oracle is not novel.");
                            ...
                            for (var i = 0; i < size; i += blockSize)
                                if (!disk.Read(regionStart + i, blockSize).AsSpan().SequenceEqual(oracle.AsSpan(i, blockSize)))
                                    throw new IOException("Performance data verification failed.");
                            ...
                            Console.WriteLine(JsonSerializer.Serialize(new
                            {
                                ...
                                flushSeconds,
                                totalSeconds,
                                dirtyAtWriteReturn = returned.DirtyBytes,
                                throttleWaits = after.ThrottleWaits - before.ThrottleWaits,
            ...
            if (args[3] == "--write-cancellation-check")
            {
                ...
                if (originalTarget.AsSpan().SequenceEqual(oracle.AsSpan(cancelledOffset, blockSize)))
                    throw new IOException("Cancellation oracle is not novel.");
                foreach (var queued in new[] { true, false })
                {
                    ...
                    for (var i = 0; i < fillBytes; i += blockSize)
                        ordinary.Write(regionStart + i, oracle.AsSpan(i, blockSize));
                    Task? barrier = null;
                    if (queued)
                    {
                        barrier = Task.Run(() =>
                        ...
                        if (!SpinWait.SpinUntil(() => OperatingSystem.IsWindows() && cache.GetWriteCacheState().Draining, 1000))
                            throw new IOException("Could not establish queued-flush cancellation window.");
                    }
                    ...
                    for (var i = 0; i < fillBytes; i += blockSize)
                        if (!ordinary.Read(regionStart + i, blockSize).AsSpan().SequenceEqual(oracle.AsSpan(i, blockSize)))
                            throw new IOException("Uncancelled prefix damaged.");
                    Console.WriteLine(JsonSerializer.Serialize(new
                    {
                        result = "PASS",
                        mode = args[3],
                        stage = queued ? "queued-behind-flush" : "waiting-for-capacity",
                        patternSeed,
                        fillBytes,
                        after,
                        utc = DateTime.UtcNow
            ...
            if (args[3] is "--write-concurrent-check" or "--write-toggle-check")
            {
                ...
                try
                {
                    ...
                    var controls = Task.Run(async () =>
                    {
                        ...
                        do
                        {
                            cache.Control(toggle ? WriteCacheAction.Disable : WriteCacheAction.Flush);
                            ++count;
                            if (toggle)
                            {
                                await Task.Delay(20);
                    ...
                    Console.WriteLine(JsonSerializer.Serialize(new
                    {
                        ...
                        workers,
                        completedBytes,
                        cachedBytes = accepted,
                        directBytes = (ulong)regionLength - accepted,
                        concurrentControlCycles = controls.Result,
                        after,
                        utc = DateTime.UtcNow
                    }));
                    return 0;
            ...
            if (args[3] == "--write-dirty-prefix")
            {
                ...
                Console.WriteLine(JsonSerializer.Serialize(new
                {
                    ...
                    prefixBytes,
                    patternSeed,
                    everyBlockNovel = true,
                    dirty,
                    explicitFlushIssued = false,
                    utc = DateTime.UtcNow
                }));
                return 0;
            }
            if (args[3] == "--write-through-check")
            {
                ...
                for (var i = 0; i < pendingBytes; i += blockSize)
                    ordinary.Write(regionStart + i, oracle.AsSpan(i, blockSize));
                var dirty = cache.GetWriteCacheState();
                if (dirty.DirtyBytes == 0 || dirty.Faulted)
                    throw new IOException("No dirty data before write-through barrier; use lab-delay.");
                ...
                Console.WriteLine(JsonSerializer.Serialize(new
                {
                    ...
                    patternSeed,
                    pendingBytes,
                    directWriteBytes = blockSize,
                    dirty,
                    after,
                    barrierSeconds = barrier.Elapsed.TotalSeconds,
                    explicitFlushIssuedByTest = false,
                    utc = DateTime.UtcNow
                }));
                return 0;
            }
            ...
            Console.WriteLine(JsonSerializer.Serialize(new
            {
                ...
                exactTransferCountsChecked = true,
...
internal sealed class RawDisk : IDisposable
{
    ...
    private void Seek(long offset, int length)
    {
        ...
        if (!Native.SetFilePointerEx(handle, offset, out var position, 0))
            throw new Win32Exception();
        if (position != offset)
            throw new IOException("Seek mismatch.");
    }
    public void Write(long offset, ReadOnlySpan<byte> data)
    {
        if (!writable)
            throw new IOException("Read-only test handle.");
        Seek(offset, data.Length);
        Marshal.Copy(data.ToArray(), 0, buffer, data.Length);
        if (!Native.WriteFile(handle, buffer, (uint)data.Length, out var count, IntPtr.Zero))
            throw new Win32Exception();
        if (count != data.Length)
            throw new IOException($"Short write: {count}/{data.Length}.");
    }
    public byte[] Read(long offset, int length)
    {
        Seek(offset, length);
        if (!Native.ReadFile(handle, buffer, (uint)length, out var count, IntPtr.Zero))
            throw new Win32Exception();
        if (count != length)
            throw new IOException($"Short read: {count}/{length}.");
    ...
    public void Flush()
    {
        if (!Native.FlushFileBuffers(handle))
            throw new Win32Exception();
    }
    public void Dispose()
    {
        handle.Dispose();
        if (buffer != IntPtr.Zero)
            Native.VirtualFree(buffer, 0, 0x8000);
    }
