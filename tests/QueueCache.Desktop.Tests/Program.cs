...
Dispatcher.UIThread.RunJobs();
Check(settings.GetVisualDescendants().OfType<NumericUpDown>().Single().IsVisible, "custom memory input appears");
settings.Close();
...
sealed class Fixture : ICacheTaskService
{
    ...
    public WriteCacheState State = new(161, 0, 200UL << 30, 4UL << 30, 4UL << 30, 1536UL << 20, 256UL << 10, 4000UL << 20, 1, 2UL << 30, 512UL << 20, 0, 0, 0, 0, 1536UL << 20);
    public int Pauses, Removes;
    public Task<IReadOnlyList<DiskDescription>> ListAsync() => Task.FromResult<IReadOnlyList<DiskDescription>>(Disks);
    public Task<WriteCacheState> ReadAsync(DiskDescription disk) => Task.FromResult(disk.Number == 1 ? State : State with { Flags = 0, BudgetBytes = 0, ReservedBytes = 0, DirtyBytes = 0, PayloadCapacity = 0 });
