...
| A09 / T032-T037, T073-T074, T080-T081, T086 | TRUE, scoped (one open issue) | Saved C: profile restore, active dirty restarts and an operator application session are verified on 0.4.117.1. | T081: 1 runtime-only and 4 saved-profile restart cycles with 83-137 MB dirty, memory pressure beyond available RAM, every byte matched, cache restored at boot. T080/T086: Paint save held in RAM while Photos opened it, identical after drain. Open: one saved-profile shutdown hang without a dump; sleep/resume, hibernate/Fast Startup and Strict restarts untested. |
