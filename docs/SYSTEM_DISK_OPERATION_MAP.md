...
| Paging, hibernation or dump path registration | `IRP_MN_DEVICE_USAGE_NOTIFICATION` reserves an in-path count under `QueueLock` before routing selection. `QcEnable` holds that lock through count validation and activation. Direct lower failure, queued cancellation/rejection and worker/lower failure undo the reservation; successful out-path completion removes it. If already routed, the request drains/disables before forwarding. Diagnostics V4 records each type's lifecycle. Plan 24 also clears `DO_POWER_PAGABLE`, invalidates/reports PnP state as not disableable, and fails query-stop/query-remove while any special-file path remains. | Enforced interim restriction: volatile caching cannot be active under one of these paths. Exact installed 0.4.78.1 reported `Paging=2`: two successful in requests from boot-time `smss.exe`, no failures and no out requests. This is balanced outstanding Windows/storage-path state, not a QueueCache decrement leak; related stacks can propagate notifications, so it does not prove two visible files. Guarded system verification now requires exact lifecycle reconciliation. | Install/reboot plan 24 and prove the PnP state remains stable. Then design nonpageable paging-I/O progress and dirty-overlap ordering before supporting active C: caching. The disabled/pass-through image baseline may run without weakening this gate. |
