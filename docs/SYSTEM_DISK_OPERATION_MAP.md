...
|---|---|---|---|
| Ordinary read/write, cache inactive | `QcDispatch` forwards directly and holds the remove lock through lower completion. Diagnostics V5 observes `IRP_PAGING_IO` reads/writes before this routing choice. | Normal pass-through; paging observation does not alter completion or ordering. | Run the plan-25 disabled 349 MiB window, then preserve in A13 compatibility qualification. |
...
| Paging, hibernation or dump path registration | `IRP_MN_DEVICE_USAGE_NOTIFICATION` reserves an in-path count under `QueueLock` before routing selection. `QcEnable` holds that lock through count validation and activation. Failure/cancellation undo the reservation; successful out-path completion removes it. Diagnostics V4 records each lifecycle. Plan 24 clears `DO_POWER_PAGABLE`, reports not-disableable state, and fails query-stop/query-remove while a path remains. | Exact installed 0.4.79.1 reconciles `Paging=2`; Windows reports the disk started/healthy and not disableable. Active caching remains excluded. Diagnostics V5 now observes actual paging data I/O separately from registration state. | Run plan 25 in disabled pass-through, then design nonpageable paging-I/O progress and dirty-overlap ordering. Do not weaken the activation gate. |
