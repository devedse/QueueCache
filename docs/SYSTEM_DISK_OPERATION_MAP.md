...
| Ordinary read/write, cache active | `QcDispatch` queues to the cancel-safe foreground worker; `QcCacheProcess` calls `Read`/`Write`. Fitting Fast writes complete from preallocated RAM. On any registered system path, up to 64 MiB stays outside ordinary admission for paging writes. | Exact 0.4.82.1 guarded Fast case passed bytes and paging progress. Plan 27 exposes the same policy through normal activation. | Exact installed plan-27 Fast/Strict and lifecycle proof; then remaining A09/T052/T053. |
| Paging, hibernation or dump path registration | Registration is reserved and ordered against Enable. Normal Enable accepts existing counts. A new in-path request drains only dirty occupancy above the ordinary limit, forwards the notification, retains clean data and keeps routing active. Failed/cancelled registration rolls its count back. | Implemented plan 27. Query-stop/remove remains correctly rejected while Windows owns a special-file path. | Exact installed registration race/state evidence, configured pagefile and hibernation/dump exercises. |
...
| Saved-profile startup | Installer task runs `qcache policy restore` as SYSTEM after a 30-second startup delay. Restore validates schema, volume, PnP identity, disk size, policy and volatile-flush acknowledgement, then uses the same unrestricted public Apply path. | Plan 27 permits a saved C: profile without guessing PhysicalDrive0. | Exact saved-profile reboot, active-state and post-restart byte evidence. |
...
now use the same paging-capable policy, and a later registration establishes its
reserve without disabling active routing.
