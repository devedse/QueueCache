...
| Ordinary read/write, cache active | `QcDispatch` queues to the cancel-safe foreground worker; `QcCacheProcess` calls `Read`/`Write`. Fitting Fast writes complete from preallocated RAM. Paging-file and unknown-origin paging data bypass admission; application paging data (file-cache write-back, mapped files) is admitted like any write since T085 (VM-verified on 0.4.111.1). | Exact 0.4.87.1 Plan-31 Fast/Strict passed complete 349 MiB byte checks and clean release. | Remaining A09 memory-pressure/application workflow and T052/T053. |
| Paging, hibernation or dump path registration | Registration is reserved and ordered against Enable. Normal Enable accepts existing counts. A new in-path request takes one drain/lower-flush boundary and invalidates clean raw blocks, but keeps routing enabled. Failed/cancelled registration rolls its count back. While the worker waits on the forwarded notification (or any forwarded PnP/shutdown request) it services queued paging reads: lower drivers may page in their handlers, and before this change that deadlocked shutdown (2026-09-26 NMI dump). | Revised after the exact 0.4.83.1 configured-pagefile failure. Query-stop/remove remains correctly rejected while Windows owns a special-file path. | Exact installed registration race/state and pagefile restart evidence. Hibernation/Fast Startup are unavailable on this VM. |
...
| Shutdown | Last-chance shutdown notification is registered. `IRP_MJ_SHUTDOWN` is queued, drains and disables through `QcShutdownBarrier`, lower-flushes, then forwards the original shutdown request. Failure is returned. Later requests keep flowing: the disabled, empty cache forwards them (Windows still pages during shutdown). Until 0.4.111.1 the suspended state failed them with `STATUS_DEVICE_NOT_READY`; T081 on 0.4.111.1 hit bugcheck 0x7A (win32kbase page-in for exiting dwm.exe) restarting with 78 MB dirty on C:. | 0.4.87.1 saved-profile reboot smoke passed; its oracle was created while inactive, not pending cached-write proof. | T081 active-write normal restart and independent bytes; T054 recovery remains open. |
| Device power down/up | Device-power transitions are queued. Leaving D0 records whether caching was active, drains/disables, lower-flushes, marks suspended and forwards. Suspension only blocks re-enabling; requests are still processed. Successful D0 clears suspension and restores the prior active state when capacity and health remain valid. | Implemented plan 27; host policy/native checks pass. | Exact sleep/resume and hibernate/Fast Startup byte/state evidence. |
...
- T085 (plan 42): a paging-marked write is admitted like any write when its
  ...
  found none below the volume and admitted nothing. 0.4.111.1 passes Q: admission and C: recognition (zero reference misses). The check runs at PASSIVE in the request worker, and the request must not
