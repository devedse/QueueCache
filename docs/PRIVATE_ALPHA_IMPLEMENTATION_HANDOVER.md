...
contract to plan 13 with the tightened `pressure` suite; VM verification is pending.
...
| A06a | PARTIAL | T049 coverage ledger and plan-13 `pressure` contract implemented with attempt timing, isolated triggers and reservation bounds. | Native/managed builds and host-safe oracle contracts pass; exact-build VM execution is pending. T050, T052-T054 and final T069 evidence remain. | Prevents known gaps and performance questions from disappearing behind completed labels. |
...
| T069 / A06a, completes T051 measurement contract | Tighten the original plan-12 `pressure` evidence before claiming exact trigger boundaries or bounded reservations. | PARTIAL in plan 13. Lower-write-attempt counters now identify start separately from `DrainedBytes` completion; explicit timing windows distinguish repeated-overwrite first-dirty age and Idle last-write age; the watermark case disables the practical age competitor and crosses from a proven-below point; capacity records dirty/in-flight/occupied-slot bounds. Host oracle contracts and native/managed builds pass. Exact-build VM evidence remains required before T051/T069 close. |
...
| V4a | Elevated test VM: `qcache developer verify Q: --suite pressure --output <off-target-results-root>` | Plan-13 focused trigger/capacity suite on the clean non-OS disk. No DiskSpd. Temporarily uses a 64 MiB cache and delay hook; restores the saved runtime configuration. Not included in `full`. |
