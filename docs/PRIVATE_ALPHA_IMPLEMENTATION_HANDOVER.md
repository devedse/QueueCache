...
| A07 | PARTIAL | T023 operation map completed; paging/hibernation/dump usage is now truthfully counted and cache activation is fail-closed while any such path exists. | Native/host checks pass; installed-driver verification pending. C: remains unqualified and T024-T027 remain. | Prevents accidental activation on known unsafe system paths while the real support work proceeds. |
