...
contract to plan 12 with the focused `pressure` suite; VM verification is pending.
...
| A06a | PARTIAL | T049 coverage ledger and plan-12 `pressure` scenario implemented. | Managed build and host-safe runner contracts pass; VM execution is pending. T050 and T052-T054 remain. | Prevents known gaps and performance questions from disappearing behind completed labels. |
| A07 | PARTIAL | T023 initial operation map and usage-path guard implemented. Review found an activation race in that guard (T068); it is not yet a proven exclusion. | Native/host/CI checks pass; kernel interleaving and installed-driver proof pending. T024-T027 and incident T067 remain. | Establishes the system-disk work needed before C: can be qualified. |
...
| T068 / A07, before relying on T025 restrictions | Repair and deterministically test usage-notification/Enable synchronization in `437ee42`. Dispatch reads `Routing` under `QueueLock`, unlocks, then `ForwardUsage` increments the count. A concurrent Enable can set routing and observe zero before that increment, allowing a direct notification past an active cache. | OPEN, source-review finding; not a demonstrated cause of T067, which predates this change. Make routing selection, pending usage reservation and activation exclusion one coherent protocol; cover success, lower failure, cancellation/removal and in/out notifications. Host scalar validation and successful builds do not prove this kernel ordering. Also define coverage for boot/system disks with no registered usage paths; the count alone is not a C: support gate. |
| T069 / A06a, completes T051 measurement contract | Tighten the plan-12 `pressure` evidence before claiming exact trigger boundaries or bounded reservations. Current tests observe completed `DrainedBytes`; Deferred may pass if draining starts after 750 ms but before its 1,000 ms setting. Idle has the analogous 350/500 ms gap. Watermark sampling can miss crossing followed by retirement; the age trigger remains enabled during the write loop. Capacity samples dirty bytes, not all reserved slots. | OPEN. Observe attempts/in-flight eligibility separately from completion, define timer tolerance and controlled preparation, isolate competing triggers, test first-dirty age under repeated overwrites, and verify slot/reservation bounds. Add host oracle contracts and exact-build VM evidence; version changed workload semantics. Current suite remains useful coarse progress/byte verification, but does not close T051. |
...
| V4a | Elevated test VM: `qcache developer verify Q: --suite pressure --output <off-target-results-root>` | Plan-12 focused trigger/capacity suite on the clean non-OS disk. No DiskSpd. Temporarily uses a 64 MiB cache and delay hook; restores the saved runtime configuration. Not included in `full`. |
